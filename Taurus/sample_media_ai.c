...
static HI_VOID YOLODetectAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn,int num_1,char* uartRead_1)
{
    HI_S32 s32Ret;
    if(GetCfgBool("yolov5_detect_switch:support_yolov5_detect",true)){
        if(g_workPlug.model==0){
            s32Ret = Yolov5DetectModelLoad(&g_workPlug.model);
            if(s32Ret < 0){
                g_workPlug.model = 0;
                SAMPLE_CHECK_EXPR_GOTO(s32Ret < 0, Detect_RELEASE,
                    "load yolo detect model err, ret=%#x\n", s32Ret);
            }
        }
        VIDEO_FRAME_INFO_S resizeFrm;
        s32Ret = MppFrmResize(&frm, &resizeFrm, YOLODet_FRM_WIDTH, YOLODet_FRM_HEIGHT);
        
        s32Ret = Yolov5DetectModelCal(g_workPlug.model, &resizeFrm, &frm,num_1,uartRead_1);
        // 将视频图像送入指定输出通道显示
        s32Ret = HI_MPI_VO_SendFrame(voLayer, voChn, &frm, 0);
        // SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, Detect_RELEASE,
        //     "HI_MPI_VO_SendFrame fail, Error(%#x)\n", s32Ret);
        MppFrmDestroy(&resizeFrm);
    }

Detect_RELEASE:
    s32Ret = HI_MPI_VPSS_ReleaseChnFrame(g_aicMediaInfo.vpssGrp, g_aicMediaInfo.vpssChn0, &frm);
    if (s32Ret != HI_SUCCESS) {
    SAMPLE_PRT("Error(%#x),HI_MPI_VPSS_ReleaseChnFrame failed,Grp(%d) chn(%d)!\n",
        s32Ret, g_aicMediaInfo.vpssGrp, g_aicMediaInfo.vpssChn0);}
}
...
static HI_S32 PauseDoUnloadYOLOModel(HI_VOID)
{
    ...
    if (GetCfgBool("yolov5_detect_switch:support_yolov5_detect", true)) {
         ...
         */
        s32Ret = Yolov5DetectModelUnLoad(g_workPlug.model);
        SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "unload cnn YOLOv5 Detect model err:%x\n", s32Ret);
        ConfBaseExt();
        g_num = 0;
    }
...
static HI_VOID* GetVpssChnFrameYOLO(HI_VOID* arg)
{
    int ret;
    int num_1=0;
    ...
    VO_LAYER voLayer = 0;
    VO_CHN voChn = 0;
    static unsigned char uartRead_1[10]={0};
    ...
    while(HI_FALSE == g_bAiProcessStopSignal){
        ...
        YOLODetectAiProcess(frm, voLayer, voChn,num_1,uartRead_1);
    }
    return HI_NULL;
}
static HI_S32 YOLOv5DetectAiThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    ...
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameYOLO, NULL);
    
    return s32Ret;
}

HI_S32 SAMPLE_MEDIA_YOLOv5_Detect(HI_VOID)
{
    HI_S32             s32Ret;
    HI_S32             fd = 0;
    //打开Sensor->VI->VPSS->VO通路
    //配置vi参数
    ViPramCfg();
    //通过Sensor型号获取enPicSize
    s32Ret = SAMPLE_COMM_VI_GetSizeBySensor(g_aicMediaInfo.viCfg.astViInfo[0].stSnsInfo.enSnsType,&g_aicMediaInfo.enPicSize);
    ...
    //创建工作线程运行ai
    s32Ret = YOLOv5DetectAiThreadProcess();
